...
for _ in range(T):
    tc = int(input())
    mat = [[0] * (N+2)] + [[0] + list(map(int, input().split())) + [0] for _ in range(N)]
    ...
    for j in start_list:
        count = 1
        end = straight(N+1, j) + [count]
        end_list.append(end)
    # print(end_list)

    min_count = end_list[0][1]
    min_index = end_list[0][0]
    for i in range(len(end_list)):
        if min_count > end_list[i][1]:
            min_count = end_list[i][1]
            min_index = end_list[i][0]

    print(f'#{tc} {min_index-1}')
