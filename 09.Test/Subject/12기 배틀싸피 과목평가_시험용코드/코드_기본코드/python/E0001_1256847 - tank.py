...
def bfs(my_position, target_position):  # 내 위치, 타겟 위치, 보유중인 포탄 수
    # [x좌표][y좌표][방향]
    ...
    while q:  # 큐가 비어있지 않는동안 반복
        ...
        if [nx,ny]==target_position:
            return path+[(direction[d], 'F S')]  # 적 전차 왔으면 발사 명령어 반환
        ...
        if 0<=nx<N and 0<=ny<N: # 맵 범위 벗어나면 다음 탐색
            if map_data[nx][ny]=='G' and not visited[nx][ny][d]:  # 다음 탐색위치가 풀이고 방문하지 않았다면
                ...
                visited[nx][ny][d]=True  # 방문 표시 후
                q.append((nx, ny, d, path+[(direction[d], 'F S')]))  #방향과 공격, 이동명령 경로에 추가하고 큐 넣기
...
while game_data is not None:
    ...
    for i in range(N):  # 맵 전체 완전탐색
        for j in range(N):
            ...
            elif map_data[i][j] == 'F': supply_position.append([i,j])  # 보급창 위치 찾고 리스트에 추가
            elif 'E' in map_data[i][j]: enemy_tank.append([i,j])  # 적 전차 위치 찾고 리스트에 추가
    
    smart=int(allies['A'][3])  # 현재 대전차탄 보유량
    etank=len(enemy_tank)  # 현재 적 전차 수

    if enemy_tank:
        # 적 전차를 목표로 잡고 경로 찾기
        for t in enemy_tank:
            path=bfs(my_position, t)
    
    # 적 탱크 존재 X일때
    # 모두 장애물 취급하고 적 포탑을 목표로 경로 찾기
    else: path=bfs(my_position, target_position)  
