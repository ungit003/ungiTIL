...
if my_pos == (5, 15):
    ...
    while game_data is not None:
        ...
        # 3번 탱크
        if cnt < 4:
            output = 'U A'  # cnt가 0, 1, 2일 경우 모두 'U A'
        else:
            output = 'U A'  # cnt가 3 이상일 경우도 'U A'로 시작
            ...
            my_pos = None
            for i in range(len(map_data)):
                for j in range(len(map_data[i])):  # 2차원 배열에서 두 번째 len 사용
                    if map_data[i][j] == 'A':
                        my_pos = (i, j)
                        break  # 찾으면 루프 종료
                if my_pos:
            ...
            if my_pos is None:
                output = 'D F S'  # 플레이어가 없을 경우 기본값 설정
            else:
                found_valid_move = False  # 유효한 이동 여부를 저장할 변수
                for elem in d:
                    dx, dy = elem[0] + my_pos[0], elem[1] + my_pos[1]
                    if 0 <= dx < len(map_data) and 0 <= dy < len(map_data[0]):  # 경계 체크
                        if map_data[dx][dy] in ['E1', 'E2', 'E3']:
                            output = d_dict[elem]
                            found_valid_move = True
                            break  # 유효한 이동이 발견되면 루프 종료

                if not found_valid_move:  # 유효한 이동이 없었다면 기본값 설정
                    output = 'D F S'
